...
create or replace procedure estadisticos_interrelacion_catastro_registro_lote_3()
language plpgsql
as $$
begin
	...
	-- *** Borrado Tablas ***
	drop table if exists tbl_interrelacion_gestor_snr_lote_3;
	drop table if exists tbl_interrelacion_levantamientocatastral_snr_lote_3;

	-- *** Borrado Vista ***
	drop view if exists vw_terrenos_interrelacion_gccatastro_registro_lote_3;
	drop view if exists vw_terrenos_interrelacion_lccatastro_registro_lote_3;

	-- Interrelación SNR\GC
	create table tbl_interrelacion_gestor_snr_lote_3 as
	(
		select (case when substring(prediocata.numero_predial,1,5) = '13458' then 'Montecristo'
				when substring(prediocata.numero_predial,1,5) = '20787' then 'Tamalameque'
				when substring(prediocata.numero_predial,1,5) = '20032' then 'Astrea'
				when substring(prediocata.numero_predial,1,5) = '13268' then 'El Peñon'
		...
		where substring(prediocata.numero_predial,1,5) in ('13458', '20787', '20032','13268')
	);	
	
	-- Interrelación SNR\LC
	create table tbl_interrelacion_levantamientocatastral_snr_lote_3 as
	(
		select (case when substring(uapre.numero_predial,1,5) = '13458' then 'Montecristo'
				when substring(uapre.numero_predial,1,5) = '20787' then 'Tamalameque'
				when substring(uapre.numero_predial,1,5) = '20032' then 'Astrea'
				when substring(uapre.numero_predial,1,5) = '13268' then 'El Peñon'
				else 'Sin información'
				end) nombre_municipio 
		...
		where substring(uapre.numero_predial,1,5) in ('13458', '20787', '20032','13268')
	); 
	
	-- *** Vistas ***
	-- Vista Terrenos \ Interrelación GC-SNR
	create view vw_terrenos_interrelacion_gccatastro_registro_lote_3 as
	(
		select row_number () over (order by inter.predio_t_id) id
			...
			,geome.geometria
		from colombiaseg_lote6.tbl_interrelacion_gestor_snr_lote_3 inter
		inner join colombiaseg_lote6.terrenos_gestor_lote_3 geome on inter.numero_predial = geome.numero_predial
	);
	...
	create view vw_terrenos_interrelacion_lccatastro_registro_lote_3 as
	(
		select row_number () over (order by inter.predio_t_id) id
			,inter.nombre_municipio 
			,inter.predio_t_id
			...
			,geome.geometria
		from colombiaseg_lote6.tbl_interrelacion_levantamientocatastral_snr_lote_3 inter
		inner join colombiaseg_lote6.terrenos_digitalizados_lote_3 geome on inter.predio_t_id = geome.predio_t_id
	);
