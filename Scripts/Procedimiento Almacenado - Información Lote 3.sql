...
create or replace procedure actualizacion_datos_lote_3()
language plpgsql
as $$
begin
	...
	-- Borrado de Tablas
	-- Temporales
	drop table if exists temp_terrenos_lote_3;
	drop table if exists temp_construcciones_lote_3;
	...
	-- Físicas
	drop table if exists terrenos_digitalizados_lote_3 cascade;
	drop table if exists terrenos_gestor_lote_3 cascade;
	drop table if exists construcciones_digitalizadas_lote_3;
	...
	drop table if exists unidades_intervencion_mh_lote_3;
	...
	-- ******************************** \\MAPHURRICANE\\ INTERRELACI�N PREDIO\TERRENO\CONSTRUCCION ********************************
	
	-- Terrenos MapHurricane - serladm
	create temp table temp_terrenos_lote_3 as
	(
		select distinct uapredio.t_id predio_t_id
			...
			,ST_Transform(ST_MakeValid(ueterreno.geometria), 9377) geometria
			...
			where substring(uapredio.numero_predial,1,5) in ('13458', '20787', '20032','13268') -- and ueterreno.area_terreno <> 0 
	);

	-- Construcciones MapHurricane - serladm
	create temp table temp_construcciones_lote_3 as
	(
		select distinct uapredio.t_id predio_t_id
			,uapredio.id_operacion predio_id_operacion
			,uapredio.numero_predial predio_numero_predial		
			...
			where substring(uapredio.numero_predial,1,5) in ('13458', '20787', '20032','13268') 
	);
	...
	create table terrenos_digitalizados_lote_3 as
	(
		...
		from temp_terrenos_lote_3 tempterrenoslote3 left join temp_unidad_intervencion tempunidadintervencion 
														on tempterrenoslote3.predio_t_id = tempunidadintervencion.id_predio
	);
	
	-- Generación Capa -Construcciones Digitalizadas-
	create table construcciones_digitalizadas_lote_3 as
	(
		select distinct predio_t_id
			,predio_id_operacion
			,predio_numero_predial		
			...
			,tempunidadintervencion.codigo codigo_unidad_intervencion
			,st_transform(geometria,9377) geometria
		from temp_construcciones_lote_3 tempconstruccioneslote3 left join temp_unidad_intervencion tempunidadintervencion 
														on tempconstruccioneslote3.predio_t_id = tempunidadintervencion.id_predio
	);
	
	-- Generación Capa -Unidades Intervención Lote 3-
	create table unidades_intervencion_mh_lote_3 as
	(
	...
	create table terrenos_gestor_lote_3 as
	(
		...
		where substring(dblinkgcprediocatastro.numero_predial,1,5) in ('13458', '20787', '20032','13268')
	);
...
/*	
	...
	select distinct uapredio.t_id predio_t_id
		...
		,(case when anotacioneslote6.estado_folio is not null then anotacioneslote6.estado_folio
			   when anotacioneslote6.estado_folio is null then 'Sin informaci�n por BD'
			   else 'Error'
			   end) estado_folio
	...
	-- (3) Asignación de orden a cada FRM Sincronizado que presenta cruce
